...
<html>
    <head>
        ...
        <link rel="icon" type="image/png" sizes="16x16" href="<?php echo APP_URL; ?>/assets/img/icons/favicon/favicon.ico">
    ...
    <!--    <section class="kr-search">
        ...
        <?php
        $UserIsNew = false;
        if ($Dashboard->_isNew() && ($Charge->_activeAbo() || $Charge->_isTrial() || $User->_isAdmin() || $User->_isManager() || !$App->_subscriptionEnabled())) {
        ...
        <?php if ($Charge->_activeAbo() || $Charge->_isTrial() || $User->_isAdmin() || $User->_isManager() || !$App->_subscriptionEnabled()): ?>
            ...
            <header>
                <div>
                            ...
                            <div class="kr-change-dashboard-selector">
                                ...
                                foreach ($listDashBoard as $numberGraph => $dashboardListType) {
                                    ...
                                    <div>
                                        <div>
                                            <span><?php
                                                if (($numberGraph - 1) == count($listDashBoard))
                                                    echo $numberGraph . ' ' . $Lang->tr('charts or more');
                                                else
                                                    echo $numberGraph . ' ' . $Lang->tr('chart') . ($numberGraph > 0 ? 's' : '');
                                                ?></span>
                                        </div>
                                        <ul>
                                            <?php
                    ...
                    <div>
                        ...
                        if (!$App->_hiddenThirdpartyActive()):
                            ...
                            if (count($listThirdParty) > 0) {
                                ...
                                foreach ($balanceList as $key => $value) {
                                    if (!is_null($balanceSelectedSymbol))
                                        continue;
                        ...
                        <?php if ($App->_hiddenThirdpartyActive() && ($App->_getTradingEnablePracticeAccount() || $App->_getTradingEnableRealAccount())): ?>
                            <div class="kr-wallet-top">
                                <div class="kr-wallet-top-<?php echo $CurrentBalance->_getBalanceType(); ?>">
                                    <div>
                                        <span><?php echo $User->_getName(); ?></span>
                                        <?php
                                        if ($App->_getBalanceEstimationShown()):
                                            //$EstimatedValueBalance = $CurrentBalance->_getEstimationBalance();
                                            $EstimatedValueBalance = $Balance->getBalanceByCoin(strtoupper($_SESSION['currency_name']));
                                            $EstimatedValueSymbol = $CurrentBalance->_getEstimationSymbol();
                                            ?>
                                            <span kr-balance-id="<?php echo $CurrentBalance->_getBalanceID(true); ?>" class="kr-wallet-top-ammount"><i id="single_balance"><?php echo $App->_formatNumber($EstimatedValueBalance, 2); ?></i> <?php echo $EstimatedValueSymbol; ?></span>
                                            <?php
                                        else:
                                ...
                                <section>
                                    <div class="kr-wallet-top-resum">
                                        <h3><?= $Lang->tr('Value of Assets'); ?><?php /*echo ucfirst($Lang->tr($CurrentBalance->_getBalanceType() . ' account'));*/ ?></h3>
                                        <ul>
                                            <?php
                                            ...
                                            foreach ($Balance->_getBalanceList() as $BalanceItem) {
                                                ...
                                                <li kr-wallet-change="<?php echo $BalanceItem->_getBalanceID(true); ?>" class="kr-wallet-top-change-<?php echo $BalanceItem->_getType(); ?>">
                                                    <div>
                                                        ...
                                                        <div>
                                                            <span id="account-balance-info"><?= $Lang->tr('Value of Assets'); ?><?php /*echo $Lang->tr($BalanceItem->_getBalanceType() . ' account');*/ ?></span>
                                                            <?php
                                                            if ($App->_getBalanceEstimationShown()):
                                                    ...
                                                    <section>
                                                        <?php if ($BalanceItem->_getType() == "real"): ?>
                                                            <?php if ($App->_getIdentityEnabled()): ?>
                                                                <?php if ($App->_getIdentityWithdrawBlocked()): ?>
                                                                    <?php if ($Identity->_identityVerified()): ?>
                                                                        <a kr-credit-widthdraw="widthdraw" class="btn btn-grey btn-autowidth btn-small"><?php echo $Lang->tr('Withdraw'); ?></a>
                                                                    <?php else: ?>
                                                                        <a onclick="_showIdentityWizard();return false;" class="btn btn-grey btn-autowidth btn-small"><?php echo $Lang->tr('Withdraw'); ?></a>
                                                                    <?php endif; ?>
                                                                <?php else: ?>
                                                                    <a kr-credit-widthdraw="widthdraw" class="btn btn-grey btn-autowidth btn-small"><?php echo $Lang->tr('Withdraw'); ?></a>
                                                                <?php endif; ?>
                                                            ...
                                                            <?php endif; ?>
                                                        <?php endif; ?>
                                                        <a kr-credit-balance="crdt" class="btn btn-<?php echo ($BalanceItem->_getType() == "practice" ? 'orange' : 'green'); ?> btn-autowidth btn-small"><?php echo $Lang->tr('Deposit'); ?></a>
                                                    </section>
                                                </li>
                                            <?php } ?>
                                        </ul>
                        ...
                        <?php
                        if ($App->_getIdentityEnabled()):
                            if ($Identity->_identityInVerifcation())
                                echo 'kr-identity-in-verification';
                            if ($Identity->_identityVerified())
                                echo 'kr-identity-verified';
                            if ($Identity->_identityWizardNotStarted())
                                echo 'kr-identity-not-verified';
                        endif;
                        ?>
                             kr-identity-acc">
                            <div class="kr-account-pic kr-user-pic-s" style="background-image:url('<?php echo $User->_getPicture(); ?>');">
                                ...
                                <?php if ($App->_getIdentityEnabled()): ?>
                                    <div>
                                        <?php
                                        if ($Identity->_identityInVerifcation())
                                            echo '<svg class="lnr lnr-undo"><use xlink:href="#lnr-undo"></use></svg>';
                                        if ($Identity->_identityVerified())
                                        ...
                                        if ($Identity->_identityWizardNotStarted())
                                            echo '<svg class="lnr lnr-cross"><use xlink:href="#lnr-cross"></use></svg>';
                                        ?>
                                    </div>
                                <?php endif; ?>
                            </div>
            ...
            <section class="kr-page-content">
                <nav class="kr-leftnav">
                    <ul>
                        <li type="module" kr-module="dashboard" kr-view="dashboard">
                            ...
                            <span><?php echo $Lang->tr('Coins'); ?></span>
                        </li>
                        <li type="module" kr-module="stock" kr-view="dashboard" kr-view-allowed="*">
                            <svg class="lnr lnr-heart-pulse"><use xlink:href="#lnr-funnel"></use></svg>
                            <span><?php echo $Lang->tr('Stocks'); ?></span>
                        </li>
                        <li type="module" kr-module="forex" kr-view="dashboard" kr-view-allowed="*">
                            <svg class="lnr lnr-heart-pulse"><use xlink:href="#lnr-funnel"></use></svg>
                            <span><?php echo $Lang->tr('Forex'); ?></span>
                        </li>
                        <?php if ((!is_null($listThirdParty) && count($listThirdParty) > 0) || $App->_hiddenThirdpartyActive()): ?>
                            <li type="side" kr-side="kr-orderbook" kr-side-part="kr-orderbook" id="orderbook-menu">
                        ...
                        <?php endif; ?>
                        <li type="module" kr-modules-hleft="true" kr-module="mining" kr-view="packagelist" kr-view-allowed="*">
                            <svg class="lnr lnr-heart-pulse"><use xlink:href="#lnr-funnel"></use></svg>
                            <span><?php echo $Lang->tr('Products'); ?></span>
                        </li>
                ...
                <section class="kr-calculatorside">
                    ...
                    <section class="kr-calculatorside-lc">
                        <?php
                        $s = 0;
                        foreach ($Calculator->_getListCurrencyUser($User) as $Symbol => $infosSymbol) {
                            $s++;
                            ?>
                            <section class="<?php echo ($s == 1 ? 'kr-calculatorside-lcsc' : ''); ?>" symbol="<?php echo $Symbol; ?>">
                                ...
                                <div>
                                    <input type="text" name="" onkeypress="return isNumberKey(event);" value="<?php echo ($s == 1 ? '1' : '0'); ?>">
                                    <span><?php echo $Symbol; ?></span>
                                </div>
                            </section>
                        <?php } ?>
            ...
            <footer>
                <div>
                    ...
                    <?php endif; ?>
                    <div class="kr-donation" onclick="_showContactPopup();">
                        <svg class="lnr lnr-bubble"><use xlink:href="#lnr-bubble"></use></svg>
                        <span><?php echo $Lang->tr('Contact us'); ?></span>
                    </div>
                    <div class="kr-donation kr-current-time" style="background: none;" mlist="<?php echo join(',', $App->_getMonthName($Lang)); ?>" dlist="<?php echo join(',', $App->_getDayName(true, $Lang)); ?>">
                        <span></span>
                    </div>
                </div>
    ...
    <script type="text/javascript" src="<?php echo APP_URL; ?>/assets/js/jquery.validate.min.js"></script>
    <script type="text/javascript" src="<?php echo APP_URL; ?>/assets/js/jquery.payform.min.js"></script>
    ...
    ?>
                        ...
                        function updateUserLastSeen() {
                            $.post('<?php echo APP_URL; ?>/app/src/User/actions/UpdateUserLastSeen.php', {}).done(function (data) {
                                //                    console.log(data);
                            }).fail(function () {
    ...
    <?php
    $Charge->_checkPaymentResult();
    if (!is_null($Balance))
        $Balance->_checkPaymentResult();
    if (isset($_GET['c']) && isset($_GET['m']) && isset($_GET['t']) && (time() - $_GET['t']) < 20)
        echo '<script>showAlert("Ooops", "' . htmlspecialchars(base64_decode($_GET['m'])) . '", "error");</script>';
    ?>
</html>
