...
try {
    ...
    if ($App->_chatIsDisabled()) {
        throw new Exception("Chat disabled.", 1);
    }
    ...
    if (empty($_POST) || !isset($_POST['room_id']) || empty($_POST['room_id']))
    ...
    if (!empty($_FILES)) {
        ...
        $extensionList = ['jpg', 'jpeg', 'gif', 'png', 'pdf', 'doc', 'docx', 'odt', 'txt', '3g2', '3gp', 'avi', 'flv', 'h264', 'm4v', 'mkv',
            'mov', 'mp4', 'mpg', 'mpeg', 'rm', 'swf', 'vob', 'wmv', 'ods', 'xls', 'xlsm', 'xlsx', 'odp', 'pps', 'ppt', 'pptx', 'aif',
            'cda', 'mp3', 'mpa', 'ogg', 'wav', 'wma', 'wpl'];
        if (!in_array($extension, $extensionList))
            throw new Exception("Error : Not valid uploaded file.", 1);
        ...
        foreach ($listDirectoryNeeded as $directoryNeeded) {
            if (!file_exists('../../../../../' . $directoryNeeded)) {
                ...
                if (!mkdir('../../../../../' . $directoryNeeded))
                    throw new Exception("Error : Fail to create directory (" . $directoryNeeded . ")", 1);
            }
        }
        //error_log(json_encode($_FILES));      

        if (!move_uploaded_file($_FILES['file']['tmp_name'], '../../../../../public/chat/' . $Room->_getRoomID(true) . '/' . $pictureName))
            throw new Exception("Error : Fail to upload picture", 1);
