...
$referaldData = [];
if($adminView == false){
    $referaldData = $User->_getReferalData();
}
        ...
        <div class="kr-user-f-l">
            <div>
                ...
                <select class="" name="kr-user-adminuserpremium">
                    <option <?php if ($Charge->_activeAbo())
            echo 'disabled="disabled"';
        if ($Charge->_isTrial() || !$Charge->_activeAbo())
            echo 'selected="selected"';
    ?> value="free">Free trial</option>
            ...
            <div>
                ...
                if ($Charge->_activeAbo())
                    $date = $Charge->_getTimestampChargeEnd();
                ?>
...
<?php endif; ?>
    <?php 
    $show = 0;
//    if(!isset($_SESSION['is_manager_login']) || $_SESSION['is_manager_login'] == false) { 
    if($show == 1) { 
        ?>
    <div class="kr-user-f-l">
        <div>
    ...
    <div class="kr-user-f-l">
        <div>
            ...
            <input type="text" name="kr_usr_phone_number" placeholder="Phone Number" value="<?php echo $User->_getPhone(); ?>">
        </div>
        <div>
            <label>Country</label>
            <?php
            ...
            if (isset($response['statuscode']) && $response['statuscode'] == '200' && isset($response['country'])) {
                ...
                foreach ($response['country'] as $country){
                    $selected = '';
                    if($User->_getCountry() == $country['countryCode']){
                        $selected = 'selected';
                    }
                    ?>
...
<?php //if(!isset($_SESSION['is_manager_login']) || $_SESSION['is_manager_login'] == false) { ?>    
...
<?php if ($referaldData['is_enable'] == 1): ?>
        <div class="kr-user-f-l">
            <div>
                <label><?php echo $Lang->tr('Referal link'); ?></label>
                <input type="text" readonly disabled value="<?php echo APP_URL; ?>/?refer_id=<?php echo $_SESSION['leads_userid']; ?>">
            </div>
        </div>
<?php endif; ?>
