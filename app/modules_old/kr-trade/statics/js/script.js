...
function _hideShowOrder(orderid){
  $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/hideShowTrade.php', {orderid:orderid}).done(function(data){
    let res = jQuery.parseJSON(data);
    if(res.error == 1){
      showAlert('Oops', res.msg, 'error');
    } else {
        var operation = (res.is_show == 1) ? "Show" : "Hide";
        var btnName = (res.is_show == 1) ? "Hide" : "Show";
      showAlert('Success', 'Order Successfully '+ operation);
      //$('[kr-orderlist-i="' + orderid + '"]').find('#hide_show_btn_'+orderid).val(operation);
      $('.hide_show_btn_'+orderid).val(btnName);
      if(res.is_show == 0){
          $('.dot_label_'+orderid).css('background-color', 'red');
          $('.dot_label_'+orderid).css('border-color', 'red');
      } else {
          $('.dot_label_'+orderid).css('background-color', 'transparent');
          $('.dot_label_'+orderid).css('border-color', 'transparent');
      }
    }
  }).fail(function(){
    showAlert('Oops', 'Fail to update order (404 or 500 error)', 'error');
  });
  return false;
}

function _hideShowTransaction(transId, type){
  $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/hideShowTransaction.php', {trans_id:transId, type:type}).done(function(data){
    let res = jQuery.parseJSON(data);
    if(res.error == 1){
      showAlert('Oops', res.msg, 'error');
    } else {
        var operation = (res.is_hide == 1) ? "Hide" : "Show";
        var btnName = (res.is_hide == 1) ? "Show" : "Hide";
      showAlert('Success', 'Transaction Successfully '+ operation);
      $('.hide_show_trans_btn_'+transId+'_'+type).val(btnName);
      if(res.is_hide == 1){
          $('.dot_label_'+transId+'_'+type).css('background-color', 'red');
          $('.dot_label_'+transId+'_'+type).css('border-color', 'red');
      } else {
          $('.dot_label_'+transId+'_'+type).css('background-color', 'transparent');
          $('.dot_label_'+transId+'_'+type).css('border-color', 'transparent');
      }
    }
  }).fail(function(){
    showAlert('Oops', 'Fail to update transaction (404 or 500 error)', 'error');
  });
  return false;
}
...
function orderBookSync(){
  ...
  $.get($('body').attr('hrefapp') + '/app/modules/kr-trade/views/orderBook.php').done(function(data){
      ...
      timeOutOrderBook = setTimeout(function(){orderBookSync();}, 5000);
  });
}
...
function _sellWithMargin(orderid, customPrice){
  $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/placeMarginTrade.php', {orderid:orderid, custom_price:customPrice}).done(function(data){
    let res = jQuery.parseJSON(data);
    if(res.error == 1){
      showAlert('Oops', res.msg, 'error');
    } else {
        showAlert('Success', 'Sell Order Successfully ');
        $('.sell_margin_div_'+orderid).hide();
        _updateBalanceData();
    }
  }).fail(function(){
    showAlert('Oops', 'Fail to update order (404 or 500 error)', 'error');
