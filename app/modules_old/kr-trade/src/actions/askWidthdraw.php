...
try {
    ...
    if(!is_null($App->_getWidthdrawCryptocurrencyAvailable())){
      ...
      // added by bhavesh
      $listWithdrawAvailable = ['paypal', 'banktransfert', 'cryptocurrencies'];
...
<section class="kr-balance-credit">
  <section style="height:480px;">
    <header>
      <span><?php echo $Lang->tr('Ask a withdraw'); ?></span>
    ...
    <section class="kr-balance-checkemail" style="display:none;">
      ...
      <h2><?php echo $Lang->tr('Your withdraw request successfully accepted'); ?></h2>
      <span><?php echo $Lang->tr('Withdraw approve within 2-3 days'); ?></span>
    </section>
  ...
  <?php endif; ?>
    ...
    <section class="kr-balance-widthdraw">
      <nav>
        <ul>
          ...
          foreach ($BalanceList as $symbol => $valueBalance) {
              if(isset($_SESSION['currency_name']) && strtoupper($_SESSION['currency_name']) != strtoupper($symbol)) continue;
      ...
      <form class="kr-createwidthdraw" style="width:0px !important;" action="<?php echo APP_URL; ?>/app/modules/kr-trade/src/actions/askWidthdrawAction.php" method="post">
        ...
        $MinimalDeposit = $App->_getMinimumWidthdraw();
        ...
        <div class="kr-balance-range-content">
          ...
          <div>
            <div class="kr-balance-range" kr-chosamount-precision="<?php echo $precision; ?>">
              <input type="text" id="kr-credit-chosamount" kr-chosamount-decimal="<?php echo $precision; ?>" kr-chosamount-step="<?php echo ($IsRealMoney ? 1 : 0.00001); ?>" kr-chosamount-symbol="<?php echo $symbolFetched;//$InfosCurrency['symbol_currency']; ?>" kr-chosamount-max="<?php echo round($BalanceListResum[$symbolFetched], $precision); ?>" kr-chosamount-min="<?php echo round($MinimalDeposit, ($IsRealMoney ? 2 : 5)); ?>" name="kr-credit-chosamount" value="" />
            </div>
          </div>
        </div>
...
<!--          <section class="kr-widthdraw-chooseacc">
        ...
        <footer style="justify-content: center;">
            ...
            <section>
              ...
              <input type="submit" class="btn btn-autowidth btn-orange" name="" value="Send Request">
            </section>
          </footer>
        <?php //else: ?>
<!--          <section class="kr-widthdraw-chooseacc-msg">
