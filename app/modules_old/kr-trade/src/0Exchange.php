...
//    $r = parent::querySqlRequest("SELECT * FROM thirdparty_crypto_krypto WHERE name_thirdparty_crypto=:name_thirdparty_crypto AND active_thirdparty_crypto=:active_thirdparty_crypto AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE status_coinslist = 1))", ['name_thirdparty_crypto' => $this->_getExchangeName(), 'active_thirdparty_crypto' => 1]);
    foreach ($r as $key => $value) {
