...
function _showPackagePopup(id){
    
    
  $.get($('body').attr('hrefapp') + '/app/modules/kr-mining/actions/viewPackagePopup.php', {id:id}).done(function(data){
    try {
      ...
      $.when($('body').prepend(data)).then(function(){
        _initPackageMethodViewController();
      });
    }
  }).fail(function(){
    showAlert('Oops', 'Fail to load view method withdraw popup','error')
  });

}

function _initPackageMethodViewController(){

}

function _buyPackage(id){
    $.get($('body').attr('hrefapp') + '/app/modules/kr-mining/actions/buyPackage.php', {id:id}).done(function(data){
      try {
        let infosJson = jQuery.parseJSON(data);
        if(infosJson.error == 1){
            showAlert('Oops', infosJson.msg, 'error');
        } else if(infosJson.success == 1){
            showAlert('Success', infosJson.msg);
            _closeContactPopup();
            $("#orderbook-menu").trigger("click");
            $("#account-balance-info").trigger("click");
        }
      } catch (e) {
        $.when($('body').prepend(data)).then(function(){
...
// Enable coin list pagination
  /*$('.kr-mining-pagination-coins').find('li').off('click').click(function(){
    changeView('mining', 'packagelist', {page:$(this).attr('kr-page')});
  });*/
function callmining_pagination(page){
    changeView('mining', 'packagelist', {"page":page,category_id:$("#search-product-category-select").val(),common_search:$("#search-product-input").val()});
}
function search_product(){
    callmining_pagination(1);
