...
try {
  ...
  $CryptoApi = new CryptoApi(null, [$_GET['currency'], $_GET['currency']], $App, (isset($_GET['market']) ? $_GET['market'] : 'US_STOCK'));
  ...
  if(!empty($_GET['t']) && $_GET['t'] == "add"){
    // Init watching list
    $StockWatchingList = new StockWatchingList($CryptoApi, $User);
    $StockWatchingList->_addItem($Coin->_getSymbol(), $_GET['currency'], (isset($_GET['market']) ? $_GET['market'] : 'CCCAGG'));
  }
...
<li class="<?= $className ?>" kr-watchinglistpair="<?php echo $_REQUEST['market'].':'.$Coin->_getSymbol().'/'.$CryptoApi->_getCurrency(); ?>">
  <div>
