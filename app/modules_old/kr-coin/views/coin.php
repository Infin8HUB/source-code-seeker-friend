...
try {
  ...
  if(empty($_REQUEST) || !isset($_REQUEST['symbol']) || empty($_REQUEST['symbol'])) throw new Exception("Error : Args missing", 1);

  if(!isset($_REQUEST['market'])) $_REQUEST['market'] = "CCCAGG";
  if(strtoupper($_REQUEST['market']) == "COINBASE") $_REQUEST['market'] = "GDAX";

  // Init crypto api
  $CryptoApi = new CryptoApi($User, [$_REQUEST['currency'], null], $App, $_REQUEST['market']);

  // Init coin associate to the graph
  $Coin = new CryptoCoin($CryptoApi, $_REQUEST['symbol'], null, $_REQUEST['market']);
  $CoinCurrency = new CryptoCoin($CryptoApi, $_REQUEST['currency'], null, $_REQUEST['market']);
  ...
  if($App->_hiddenThirdpartyActive()){
    ...
    if($App->_hiddenThirdpartyActive()){
      $listThirdParty = $Trade->_thirdparySymbolTrading($Coin->_getSymbol(), $CryptoApi->_getCurrency(), $_REQUEST['market']);
    } else {
      $listThirdParty = $Trade->_thirdparySymbolTrading($Coin->_getSymbol(), $CryptoApi->_getCurrency(), $_REQUEST['market']);
    }
    $availableTrading = count($listThirdParty) > 0;
    if(count($listThirdParty) > 0) $availableTrading = $listThirdParty[0];
//    echo "<pre>"; print_r($availableTrading); exit;
  } else {
    $availableMarketGiven = null;
    if(isset($_REQUEST['market'])){
      $availableMarketGiven = $Trade->_thirdparySymbolTrading($_REQUEST['symbol'], $_REQUEST['currency'], $_REQUEST['market']);
      if(count($availableMarketGiven) > 0) $availableTrading = $availableMarketGiven[0];
    } else {
      $availableMarketGiven = $Trade->_thirdparySymbolTrading($_REQUEST['symbol'], $_REQUEST['currency']);
      $availableTrading = $availableMarketGiven[0];
    }

    if($availableMarketGiven != null && !isset($_REQUEST['market'])){
      $listMarketAvailable = $availableMarketGiven;
    } else {
      $listMarketAvailable = $Trade->_thirdparySymbolTrading($_REQUEST['symbol'], (isset($_REQUEST['currency']) ? $_REQUEST['currency'] : $CryptoApi->_getCurrency()));
  ...
  if(is_null($availableTrading) && $App->_hiddenThirdpartyActive()){
    ...
    //$Coin = new CryptoCoin($CryptoApi, $_REQUEST['symbol']);
  }
    ...
    <div style="<?php echo (is_null($availableTrading) ? 'width:100%;' : (is_null($OrderBook) ? 'width:85%;' : '')); ?>">
      ...
      <section class="kr-cinf-order">
        <section>
          ...
          <?php if(isset($availableTrading) && $availableTrading->_isActivated()): ?>
