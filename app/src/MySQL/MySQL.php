...
class MySQL {
	...
	public static function execSqlRequest($query, $def = []){
        ...
        /**
	 * Execute SQL Request (INSERT)
	 * @param  String          $query SQL Query ("INSET INTO")
	 * @param  Array          $def   SQL Def ['id_key' => 'xxxx', ...]
	 *
	 * @return Boolean                True = Last insert id, False = Fail SQL
	 */
	public static function insertSqlRequest($query, $def = []){
		$req= self::getSqlConnexion()->prepare($query);
		$req->execute($def);
                $lastInsertId = self::getSqlConnexion()->lastInsertId();
		$req->closeCursor();
		return $lastInsertId;
	}
