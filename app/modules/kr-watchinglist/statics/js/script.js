...
function toggleWatchingList(symbol, currency, market = "CCCAGG") {
    ...
    if ($('[kr-watchinglistpair="' + market.toUpperCase() + ":" + symbol + '/' + currency + '"]').length > 0)
        ...
        addWatchingListItem(symbol, currency, "add", false, market); // Else add watching list item
}
...
function addWatchingListItem(symbol, currency, type = "load", first = false, market = "CCCAGG") {
    ...
    var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-watchinglist/src/actions/getWatchingItem.php';
    if(market == 'US_STOCK' || market == 'usstock'){
        var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-stock/src/actions/getStockWatchingItem.php';
    }
    if(market.toLowerCase() == 'forex'){
        var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-forex/src/actions/getForexWatchingItem.php';
    }
    ...
    if (market != "CCCAGG")
        market = market.substr(0, 1).toUpperCase() + market.substr(1).toLowerCase();
...
function removeWatchingListItem(symbol, currency, market) {
    ...
    // Remove item in DB
    var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-watchinglist/src/actions/removeWatchingListItem.php';
    if(market == 'US_STOCK' || market == 'usstock'){
        var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-stock/src/actions/removeStockWatchingListItem.php';
    }
    if(market == 'FOREX' || market == 'forex'){
        var postUrl = $('body').attr('hrefapp') + '/app/modules/kr-forex/src/actions/removeForexWatchingListItem.php';
    }
    $.post(postUrl, {
        symb: symbol,
        currency: currency,
