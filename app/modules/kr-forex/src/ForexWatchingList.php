 ...
 */
class ForexWatchingList extends MySQL {
   ...
   */
  public function _getListsForex(){
    $resCoin = [];
    $r = parent::querySqlRequest("SELECT * FROM forex_watching_krypto WHERE id_user=:id_user", ['id_user' => $this->_getUser()->_getUserID()]);
    if(count($r) > 0){
        ...
        $r = parent::querySqlRequest("SELECT symbol, currency, displaySymbol FROM forexlist_krypto WHERE status=1 LIMIT 5");
        if(count($r) > 0){
            foreach ($r as $key => $itemWatching) {
                $resCoin[$itemWatching['displaySymbol']] = [
                  'symbol' => $itemWatching['symbol'],
                  'currency' => $itemWatching['currency'],
                  'market' => 'Forex'
                ];
   ...
   * @return Boolean
   */
  public function _removeItem($symbol, $currency, $market = "Forex"){

    // Delete item in Database
    $r = parent::execSqlRequest("DELETE FROM forex_watching_krypto WHERE symbol=:symbol AND currency=:currency AND market=:market AND id_user=:id_user", [
                                'symbol' => $symbol,
                                'currency' => $currency,
   ...
   * @param String $symbol Symbol item (ex : BTC)
   */
  public function _addItem($symbol, $currency = null, $market = "Forex"){

    if(is_null($currency)) $currency = $this->_getCryptoApi()->_getCurrencyFullName();

    // Check if item is alreayd in watching list
    $r = parent::querySqlRequest("SELECT * FROM forex_watching_krypto WHERE symbol=:symbol AND currency=:currency AND market=:market AND id_user=:id_user", [
                                'symbol' => $symbol,
                                'id_user' => $this->_getUser()->_getUserID(),
                                'market' => $market,
                                'currency' => $currency
    ...
    if(count($r) == 0){
      ...
      $s = parent::execSqlRequest("INSERT INTO forex_watching_krypto (symbol, id_user, currency, market) VALUES (:symbol, :id_user, :currency, :market)",
                                  [
                                    'symbol' => $symbol,
                                    'id_user' => $this->_getUser()->_getUserID(),
                                    'market' => $market,
  ...
  public function _symbolExist($symbol, $currency){
    $r = parent::querySqlRequest("SELECT * FROM forex_watching_krypto WHERE symbol=:symbol AND currency=:currency AND id_user=:id_user", [
                                'symbol' => $symbol,
                                'currency' => $currency,
                                'id_user' => $this->_getUser()->_getUserID()
                              ]);
