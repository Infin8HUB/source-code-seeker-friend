 ...
 */
function addStockCoinValue(value, symbol) {

    let chart = graphStockListView[symbol];
 ...
 */
function checkStockWatchingListSymbol() {
    $.get($('body').attr('hrefapp') + '/app/modules/kr-stock/src/actions/getStockWatchingListSymbol.php').done(function (data) {
        ...
        if (respond.error == 0) {
            $.each(respond.item, function (k, v) {
                setTimeout(function () {
                    ...
                    addStockWatchingListItem(v.symbol, v.currency, "load", true, v.market);
                }, 1);
            });
        } else {
 ...
 * @param  {String} symbol Coin symbol
 */
function updateStockWatchCoinItem(data, symbol, currency, market = "CCCAGG") {
 ...
 */
function toggleStockWatchingList(symbol, currency, market = "US_STOCK") {
    // If watching list item found = remvoe
    if ($('[kr-watchinglistpair="' + market.toUpperCase() + ":" + symbol + '/' + currency + '"]').length > 0)
        removeStockWatchingListItem(symbol, currency, market);
    else
        addStockWatchingListItem(symbol, currency, "add", false, market); // Else add watching list item
}

function addStockWatchingListItem(symbol, currency, type = "load", first = false, market = "CCCAGG") {
    if ($('[kr-watchinglistpair="' + market.toUpperCase() + ":" + symbol + '/' + currency + '"]').length > 0)
        return false;
    $('.kr-dash-pan-cry-select-lst-tdn[symbol="' + symbol + '"]').addClass('watching-list-present');

    // Get watching list item data
    $.get($('body').attr('hrefapp') + '/app/modules/kr-stock/src/actions/getStockWatchingItem.php', {
        symb: symbol,
        currency: currency,
        market: market,
        t: type
...
//            updateStockWatchCoinItem(dataCoin, dataCoin.FROMSYMBOL, dataCoin.TOSYMBOL, dataCoin.MARKET.toUpperCase());
//        }
//
...
/**
 * Remove stock wathcing item
 * @param  {String} symbol Item symbol (ex : BTC)
 */
function removeStockWatchingListItem(symbol, currency, market) {
    ...
    // Remove item in DB
    $.post($('body').attr('hrefapp') + '/app/modules/kr-stock/src/actions/removeStockWatchingListItem.php', {
        symb: symbol,
        currency: currency,
        market: market
    }).done(function (data) {
