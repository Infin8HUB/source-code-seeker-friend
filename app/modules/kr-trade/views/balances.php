...
try {
    ...
    $CryptoApi = new CryptoApi($User, ['USDT', 'USDT'], $App);
    ...
    if ($App->_hiddenThirdpartyActive()) {
        ...
        $BitcoinBalanceEstimation = $Balance->_getEstimationBalance('USDT');
        $BalanceEstimationSymbol = $Balance->_getEstimationSymbol();
        ...
        echo "Test" . $selectedThirdParty = $listThirdParty[0];
        exit;

        $BalanceList = $selectedThirdParty->_getBalance(true);

        $BitcoinBalanceEstimation = $selectedThirdParty->_getBalanceEstimation('USDT', $Balance);
...
<section class="kr-balance-view">
    <header>
        ...
        <div>
            <div>
                ...
                <div class="kr-balances-view-tggsmall" title="<?php echo $Lang->tr('Balances valued less than 0.001 USDT'); ?>">
                    <label><?php echo $Lang->tr('Hide small balance'); ?></label>
                </div>
            </div>
            <span><?php echo $Lang->tr('Estimate value'); ?> : <b><?php echo $App->_formatNumber($BitcoinBalanceEstimation, 8); ?> USDT / <?php echo $App->_formatNumber($BalanceEstimation, 2) . ' ' . $BalanceEstimationSymbol; ?> </b></span>
        </div>
    </header>
    ...
    <div class="kr-marketlist">
        <div class="kr-marketlist-header">
            ...
            <?php endif; ?>
            <div class="kr-mono"><span><?php echo $Lang->tr('Value in') . ' USDT'; ?></span></div>
        ...
        if ($App->_hiddenThirdpartyActive()) {
            ...
            foreach ($Balance->_getBalanceListResum() as $key => $value) {
                ...
                if ($Balance->_symbolIsMoney($key)) {
                    ...
                    if ($value > 0) {
                        $SymbolConvertPrice = $Balance->_convertCurrency($value, $key, 'USDT');
                        $SymbolConvertPriceIndicatif = $Balance->_convertCurrency($SymbolConvertPrice, 'USDT', $Balance->_getEstimationSymbol(true));
                    }
                } else {
                    try {
                        if ($Balance->_symbolIsStock($key)) {
                            $CryptoApi = new CryptoApi($User, ['USDT', 'USDT'], $App, 'US_STOCK');
                        } else if ($Balance->_symbolIsForex($key)) {
                            $CryptoApi = new CryptoApi($User, ['USDT', 'USDT'], $App, 'FOREX');
                        } else {
                            $CryptoApi = new CryptoApi($User, ['USDT', 'USDT'], $App);
                        }
                        ...
                        $SymbolConvertPrice = 0;
                        if ($value > 0)
                            $SymbolConvertPrice = $Coin->_convertTo('USDT', $value);

                        $SymbolConvertPriceIndicatif = 0;
                        if ($value > 0)
                            $SymbolConvertPriceIndicatif = $Coin->_convertTo($Balance->_getEstimationSymbol(true), $SymbolConvertPrice, 'USDT');
                    } catch (\Exception $e) {
                        continue;
                    }
                }
                
                if(!in_array($key, $activeCoinList)){
                    continue;
                }
                ?>
                <div class="kr-marketlist-item kr-balanceitem-cv" kr-balance-item-value-currency="<?php echo $key; ?>" kr-balance-item-value="<?php echo $SymbolConvertPrice; ?>">
                    ...
                    <div class="kr-mono">
                        <span><?php echo $App->_formatNumber($SymbolConvertPrice, 8); ?> USDT</span>
                    </div>
                    <div class="kr-mono">
        ...
        <?php if (in_array($key, $DepositSymbolAllowed)): ?>
                        ...
                        <?php if (isset($_SESSION['currency_name']) && strtoupper($_SESSION['currency_name']) == strtoupper($key)): ?>
