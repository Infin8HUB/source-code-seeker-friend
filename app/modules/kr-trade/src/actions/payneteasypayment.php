...
//    print_r($responseData); exit;
    if(isset($gateWayData['gateway']) && in_array('paynet', $gateWayData['gateway'])){  
        if(isset($responseData['statuscode']) && $responseData['statuscode'] == '200'){    
            if(!empty($paymentSetting['setting']['paynet_group_id']) && !empty($paymentSetting['setting']['paynet_merchant_control'])){
                ...
                $groupId = $paymentSetting['setting']['paynet_group_id']; 
                $merchantControl = $paymentSetting['setting']['paynet_merchant_control'];
                
                $apiUrl = "https://sandbox.payneteasy.eu/paynet/api/v2/sale-form/group/".$groupId;
                if(isset($paymentSetting['setting']['paynet_payment_mode']) && $paymentSetting['setting']['paynet_payment_mode'] == 'live'){
                    $apiUrl = "https://gate.payneteasy.eu/paynet/api/v2/sale-form/group/".$groupId;
                }
...
//                    'state' => (!empty($_POST['state'])) ? $_POST['state'] : $userData['state'], 
                    ...
                    'server_callback_url' => 'https://leads8.com/dmn/paynet_callback/',
