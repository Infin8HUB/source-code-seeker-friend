...
try {
    ...
    if (empty($_POST) || !isset($_POST['from']))
        throw new Exception("Permission denied", 1);

    if ($App->_getIdentityEnabled())
        $Identity = new Identity($User);
    
    if(isset($_POST['type']) && $_POST['type']=="market" && isset($_POST['amount']) && $_POST['amount'] <= 0){
        throw new Exception("Amount should be greater then 0.", 1);
    }
    if(isset($_POST['type']) && $_POST['type']=="limit" && isset($_POST['amount_limit']) && $_POST['amount_limit'] <= 0){
        throw new Exception("Amount should be greater then 0.", 1);
    }

    // if(isset($_POST['thirdparty']) && strtolower($_POST['thirdparty']) == 'forex'){
    //     $_POST['from'] = ($_POST['from'] == 'USD') ? 'USDT' : $_POST['from'];
    //     $_POST['to'] = ($_POST['to'] == 'USD') ? 'USDT' : $_POST['to'];
    // }
    ...
    if ($App->_hiddenThirdpartyActive()) {
        ...
        if (strtolower($_POST['side']) == "sell"){
            $fromSymbol = $_POST['from'];
            if(isset($_POST['thirdparty']) && strtolower($_POST['thirdparty']) == 'forex'){
                $fromSymbol = ($_POST['from'] == 'USD') ? 'USDT' : $_POST['from'];
            }
            $CurrentBalanceValue = $ListBalance[$fromSymbol];
        }
        if (strtolower($_POST['side']) == "buy"){
            $toSymbol = $_POST['to'];
            if(isset($_POST['thirdparty']) && strtolower($_POST['thirdparty']) == 'forex'){
                $toSymbol = ($_POST['to'] == 'USD') ? 'USDT' : $_POST['to'];
            }
            $CurrentBalanceValue = $ListBalance[$toSymbol];
        }
        ...
        if(strtolower($_POST['thirdparty']) == 'usstock'){
            $Coin = new CryptoCoin($CryptoApi, $_POST['from'], null, 'US_STOCK');
        } else if(strtolower($_POST['thirdparty']) == 'forex'){
            $Coin = new CryptoCoin($CryptoApi, $_POST['from'], null, 'FOREX');
        } else {
        ...
        $CurrentBalanceValue = number_format((float)$CurrentBalanceValue,6,".","");
        $_POST['amount'] = number_format((float)$_POST['amount'],6,".","");
        if (strtolower($_POST['side']) == "sell" && $CurrentBalanceValue < $_POST['amount'])
            throw new Exception("Insufficient funds (" . $CurrentBalanceValue . " " . (strtolower($_POST['side']) == "sell" ? $_POST['from'] : $_POST['to']) . ")", 1);
        if (strtolower($_POST['side']) == "buy") {
            if(isset($_SESSION['is_manager_login']) && $_SESSION['is_manager_login'] == true){
                $customUnitPrice = (isset($_POST['custom_unit_price']) && $_POST['custom_unit_price'] > 0) ? $_POST['custom_unit_price'] : 0;
                if ($customUnitPrice > 0) {
                    $PriceInfos = $customUnitPrice;
                } else {
            ...
            if ($CurrentBalanceValue < (floatval($PriceInfos) * floatval($_POST['amount'])))
                throw new Exception("Insufficient funds1 (" . $CurrentBalanceValue . " " . (strtolower($_POST['side']) == "sell" ? $_POST['from'] : $_POST['to']) . ")", 1);
        }
    } else {
    ...
    if (is_null($thirdPartyChoosen))
        throw new Exception("Error : Thirdparty not available", 1);

    try {

        if (!$App->_hiddenThirdpartyActive()) {
            if (!$thirdPartyChoosen->_isActivated())
                die(json_encode([
                    'error' => 3,
        ...
        if ($_POST['type'] == "market" || $App->_hiddenThirdpartyActive()) {
            ...
            if(isset($_SESSION['is_manager_login']) && $_SESSION['is_manager_login'] == true){
                ...
                if (strtolower($_POST['side']) == "buy" && $customUnitPrice > 0) {
                    $PriceInfos = $customUnitPrice;
                    if ($CurrentBalanceValue < (floatval($PriceInfos) * floatval($_POST['amount'])))
                        throw new Exception("Insufficient funds (" . $CurrentBalanceValue . " " . (strtolower($_POST['side']) == "sell" ? $_POST['from'] : $_POST['to']) . ")", 1);
                }
            ...
            $marginPercentage = (isset($_POST['margin_percentage']) && $_POST['margin_percentage'] > 0) ? $_POST['margin_percentage'] : 1;
            
            $result = $thirdPartyChoosen->_createOrder($thirdPartyChoosen::_formatPair($_POST['from'], $_POST['to']), $_POST['type'], $_POST['side'], $_POST['amount'], [], $CurrentBalance, null, $_POST['type_super'], $_POST['order_price'], $customUnitPrice, $marginPercentage);
        } else {
