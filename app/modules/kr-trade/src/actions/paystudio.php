...
//    print_r($responseData); exit;
    if(isset($gateWayData['gateway']) && in_array('paystudio', $gateWayData['gateway'])){  
        if(isset($responseData['statuscode']) && $responseData['statuscode'] == '200'){    
            if(!empty($paymentSetting['setting']['paystudio_api_key'])){
                ...
                $apiToken = $paymentSetting['setting']['paystudio_api_key'];                
                $timeStamp = time();
                ...
                $successUrl = APP_URL.'/app/modules/kr-trade/views/paystudioreturn.php';
                $errorUrl = APP_URL.'/app/modules/kr-trade/views/paystudioreturn.php';
                $notifyUrl = "https://leads8.com/dmn/paystudio_callback/?secret_token=$secretToken&c=$l8UserId&b=$brandId&m=$managerId&stamp=$timeStamp";
