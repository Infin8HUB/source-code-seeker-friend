...
try {
    ...
    if(!empty($orderData)){
        ...
        foreach ($orderData as $orderInfo){
            ...
            if(strtolower($orderInfo['thirdparty_internal_order']) == 'forex'){
                $symbol_internal_order = ($orderInfo['symbol_internal_order'] == 'USDT') ? 'USD' : $orderInfo['symbol_internal_order'];
                $to_internal_order = ($orderInfo['to_internal_order'] == 'USDT') ? 'USD' : $orderInfo['to_internal_order'];
                $coinPair = $symbol_internal_order."/".$to_internal_order;
            }
