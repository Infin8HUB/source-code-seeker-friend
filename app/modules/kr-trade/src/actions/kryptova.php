...
//    print_r($responseData); exit;
    if(isset($gateWayData['gateway']) && in_array('kryptova', $gateWayData['gateway'])){  
        if(isset($responseData['statuscode']) && $responseData['statuscode'] == '200'){    
            if(!empty($paymentSetting['setting']['kryptova_api_key'])){
                ...
                $apiUrl = "https://hello.kryptova.biz/api/hosted-pay/payment-request";
                $apiToken = $paymentSetting['setting']['kryptova_api_key'];                
                $timeStamp = time();
                ...
                $successUrl = APP_URL.'/app/modules/kr-trade/views/kryptovareturn.php';
                $errorUrl = APP_URL.'/app/modules/kr-trade/views/kryptovareturn.php';
                $notifyUrl = "https://leads8.com/dmn/kryptova_callback/?secret_token=$secretToken&c=$l8UserId&b=$brandId&m=$managerId&stamp=$timeStamp";
