...
//    print_r($responseData); exit;
    if(isset($gateWayData['gateway']) && in_array('chargemoney', $gateWayData['gateway'])){  
        if(isset($responseData['statuscode']) && $responseData['statuscode'] == '200'){    
            if(!empty($paymentSetting['setting']['chargemoney_api_key'])){
                ...
                //$apiUrl = "https://dashboard.paystudio.app/api/hosted-pay/payment-request";
                $apiUrl = "https://dashboard.charge.money/api/hosted-pay/payment-request";
                $apiToken = $paymentSetting['setting']['chargemoney_api_key'];                
                $timeStamp = time();
                ...
                $successUrl = APP_URL.'/app/modules/kr-trade/views/chargemoneyreturn.php';
                $errorUrl = APP_URL.'/app/modules/kr-trade/views/chargemoneyreturn.php';
                $notifyUrl = "https://leads8.com/dmn/chargemoney_callback/?secret_token=$secretToken&c=$l8UserId&b=$brandId&m=$managerId&stamp=$timeStamp";
                ...
                if(isset($responseData['status']) && $responseData['status'] == 'fail'){
                    ...
                    if(isset($responseData['redirect_3ds_url']) && $responseData['redirect_3ds_url'] != ''){
