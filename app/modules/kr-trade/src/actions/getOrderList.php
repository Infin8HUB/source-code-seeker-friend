...
try {
    ...
    if(!$App->_hiddenThirdpartyActive()){
      ...
      if(isset($_GET['market']) && strtoupper($_GET['market']) == 'FOREX'){
        $symbol = ($_GET['symbol'] == 'USD') ? 'USDT' : $_GET['symbol'];
        $currency = ($_GET['currency'] == 'USD') ? 'USDT' : $_GET['currency'];
      }
      $BookListNF = $CurrentBalance->_getOrderHistory(null, $symbol, $currency);
      foreach ($BookListNF as $key => $value) {
        ...
        $symbol_internal_order = $value['symbol_internal_order'];
        $to_internal_order = $value['to_internal_order'];
        if(isset($value['thirdparty_internal_order']) && strtolower($value['thirdparty_internal_order']) == 'forex'){
          $symbol_internal_order = ($value['symbol_internal_order'] == 'USDT') ? 'USD' : $value['symbol_internal_order'];
          $to_internal_order = ($value['to_internal_order'] == 'USDT') ? 'USD' : $value['to_internal_order'];
        }

        $CurrentPrice = $Balance->_convertCurrency(1, $symbol_internal_order, $to_internal_order, $value["thirdparty_internal_order"]);
        ...
        $marginVal = 1;
        $buyCoinBalance = $Balance->getBalanceByCoin($value['symbol_internal_order']);
        if($buyCoinBalance > 0 && $value['side_internal_order'] == 'BUY' && $value['margin_val'] > 1 && $value['is_sold'] == 0){
            $marginVal = (int)$value['margin_val'];
        }

        if(isset($value['thirdparty_internal_order']) && strtolower($value['thirdparty_internal_order']) == 'forex'){
          $value['symbol_internal_order'] = ($value['symbol_internal_order'] == 'USDT') ? 'USD' : $value['symbol_internal_order'];
          $value['to_internal_order'] = ($value['to_internal_order'] == 'USDT') ? 'USD' : $value['to_internal_order'];
        }

        $ratio = '';
        $leverageAmount = '';
        if($value['side_internal_order'] == 'SELL'){
            $ratio = 'Ratio: '.($value['usd_amount_internal_order'] - $value['margin_amount']) .' '. $value['to_internal_order'];
            if($value['margin_val'] > 1){
                if($value['margin_amount'] > 0){
                    $leverageAmount = '<br>Leverage x'.(int)$value['margin_val'].': <span style="color:green">'.$value['margin_amount'].' '.$value['to_internal_order'].'</span>';
                }
                else if($value['margin_amount'] < 0){
                    $leverageAmount = '<br>Leverage x'.(int)$value['margin_val'].': <span style="color:red">'.$value['margin_amount'].' '.$value['to_internal_order'].'</span>';
                } else {
                    $leverageAmount = '<br><span>Leverage x'.(int)$value['margin_val'].': '.$value['margin_amount'].' '.$value['to_internal_order'].'</span>';
                }
            }
        } else {
            $ratio = $value['amount_internal_order'] .' '.$value['to_internal_order'];
        }
        $details = $ratio.$leverageAmount;
        ...
        $BookList[$value['id_internal_order']] = [
          ...
          'evolv' => ($DiffOrder > 0 ? '+' : '').''.rtrim($App->_formatNumber($DiffOrder, ($DiffOrder > 1 ? 2 : ($DiffOrder < -1 ? 2 : 6)))).' '.
            ...
            'manager_login' => ($_SESSION['is_manager_login'] == true) ? 1 : 0,
                'is_show' => $value['is_show'],
            'margin' => (int)$value['margin_val'],
            'is_sold' => $value['is_sold'],
