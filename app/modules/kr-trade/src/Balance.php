  ...
  public function _saveOrder($exchange, $amount, $usd_total, $side, $symbol, $order, $to, $type = "market", $ordered_price = null, $marginPercentage = 1, $marginText = '', $refOrderId = 0, $marginAmount = 0){
    ...
    $r = parent::insertSqlRequest("INSERT INTO internal_order_krypto (id_user, date_internal_order, id_balance, thirdparty_internal_order, amount_internal_order,
                                  ...
                                  [
                                    ...
                                    'symbol_internal_order' => (($symbol == 'USD') ? 'USDT' : $symbol),
                                    'side_internal_order' => $side,
                                    'order_key_internal_order' => ($this->_isPractice() ? '' : $order['id']),
                                    'fees_internal_order' => $fees,
                                    'to_internal_order' => (($to == 'USD') ? 'USDT' : $to),
...
//                $betAmount = $this->_convertCurrency($sellAmount, $sellCurrency, $brandCurrency);
            ...
            $buyCurrency = ($buyCurrency == 'USD') ? 'USDT' : $buyCurrency;
            $sellCurrency = ($sellCurrency == 'USD') ? 'USDT' : $sellCurrency;
...
//                $response = $leadsApiObj->callCurl('updateCustomerCreditMinus', $sellParam);
  ...
  public function _getBalanceListResum(){
    ...
    // echo "<pre>"; print_r($activeCoinList); exit;
  ...
  public function _symbolIsForex($symbol){
    $r = parent::querySqlRequest("SELECT symbol FROM forexlist_krypto WHERE symbol=:symbol AND status = 1", ['symbol' => $symbol]);
    return count($r) > 0;
  }
  ...
  //  print_r($BalanceData); exit;
    ...
    $ForexSymbolListAvailableBalance = [];
    ...
    $conv = [];
    if(count($SymbolListAvailableBalance) > 0){
    ...
    $convForex = [];
    if(count($ForexSymbolListAvailableBalance) > 0){
      $convForex = $CryptoApi->_getData('price', ['fsym' => (is_null($tosymbol) ? $estimatedSymbol : $tosymbol), 'tsyms' => join(',', $ForexSymbolListAvailableBalance)], 'forex-api');
    }
    $res = 0;
...
//    }
    ...
    foreach ($convForex as $key => $value) {
      $res += (1 / $value) * $BalanceData[$key];
    }
  ...
  public function getActiveCoinList(){
        ...
        $rForex = parent::querySqlRequest("SELECT DISTINCT symbol FROM `forexlist_krypto` WHERE `status` = 1");
        $resForex = [];
        foreach ($rForex as $key => $value) {
            $resForex[] = $value['symbol'];
        }   

        $rForexCurrency = parent::querySqlRequest("SELECT DISTINCT currency FROM `forexlist_krypto` WHERE `status` = 1");
        $resForexCurrency = [];
        foreach ($rForexCurrency as $key => $value) {
            $resForexCurrency[] = $value['currency'];
        }   

        $res = array_merge($res, $resStock);
        $res = array_merge($res, $resForex);
        $res = array_merge($res, $resForexCurrency);
        return array_values($res);
  }
  ...
  public function _saveMining($amount, $symbol, $product_id, $type = "mining"){
    ...
    $r = parent::insertSqlRequest("INSERT INTO internal_order_krypto (id_user, date_internal_order, id_balance, thirdparty_internal_order, amount_internal_order,
                                  ...
                                  [
                                    ...
                                    'to_internal_order' => (($symbol == 'USD') ? 'USDT' : $symbol),
                                    'ref_internal_order' => $orderRef,
                                    'type_internal_order' => $type,
                                    'status_internal_order' => 1,
