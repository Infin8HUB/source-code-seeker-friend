...
function processSearch(searchBox, request, searhcfieldcontent = null){
    var selectedModule = $('.kr-leftnav-select').attr('kr-module');
  SearchServiceRequest = $.get($('body').attr('hrefapp') + '/app/modules/kr-search/src/actions/searchQuery.php', {request:request, module:selectedModule}).done(function(data){
    ...
    if(requestData.error == 0){
      ...
      $.each(requestData.coinlist, function(k, item){
        ...
        let itemSearch;
        if(selectedModule == 'stock'){
            itemSearch = $('<li>' +
          '<div>' +
            '<span>' + item.symbol_exchanges + '</span>' +
          '</div>' +
          '<div>' +
            '<span>' + (item.symbol_longname == null ? item.symbol_exchanges : item.symbol_longname) + '</span>' +
          '</div>' +
          '<div>' +
            '<span>' + (requestData.native == 0 ? item.market_exchanges : '') + '</span>' +
          '</div>' +
        '</li>');
        } else if(selectedModule == 'forex'){
            itemSearch = $('<li>' +
          '<div>' +
            '<span>' + item.symbol_exchanges + '</span>' +
          '</div>' +
