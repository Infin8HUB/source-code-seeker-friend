...
class Search extends MySQL {
      ...
      $r = parent::querySqlRequest("SELECT market_exchanges, symbol_exchanges, currency_exchanges,
                                    (
                                      ...
                                      WHERE coinlist_krypto.symbol_coinlist = exchanges_krypto.symbol_exchanges
                                       AND coinlist_krypto.status_coinslist = 1
                                    ) AS symbol_longname,
                                    (
                                    ...
                                    AND (EXISTS (SELECT id_coinlist FROM coinlist_krypto WHERE coinlist_krypto.symbol_coinlist=exchanges_krypto.symbol_exchanges AND coinlist_krypto.status_coinslist = 1)
                                        OR EXISTS (SELECT id_currency FROM currency_krypto WHERE currency_krypto.code_iso_currency=exchanges_krypto.symbol_exchanges AND currency_krypto.status_currency = 1))
                                    AND (EXISTS (SELECT id_coinlist FROM coinlist_krypto WHERE coinlist_krypto.symbol_coinlist=exchanges_krypto.currency_exchanges AND coinlist_krypto.status_coinslist = 1)
                                        OR EXISTS (SELECT id_currency FROM currency_krypto WHERE currency_krypto.code_iso_currency=exchanges_krypto.currency_exchanges AND currency_krypto.status_currency = 1))
  ...
  public function _query1($search, $limit = 250){
    $resQuery = [];
    $alreadyFetched = '';
    foreach ($this->_getFromCache() as $key => $value) {
      if(in_array($value['market_exchanges'].":".$value['symbol_exchanges'].$value['currency_exchanges'], $alreadyFetched)) continue;
      $alreadyFetched .= $value['symbol_exchanges'].'/'.$value['currency_exchanges']."<br>";
      $resQuery[] = $value;
    }
    echo $alreadyFetched;
    exit;
  ...
  public function _Stockquery($search, $limit = 250){
        $resQuery = [];
        $alreadyFetched = [];
//        echo "SELECT description, symbol FROM stocklist_krypto WHERE status = 1 AND (symbol LIKE '%'$search'%' OR description LIKE '%'$search'%'";
        $r = parent::querySqlRequest("SELECT description, symbol FROM stocklist_krypto WHERE status = 1 AND (symbol LIKE :symbol OR description LIKE :description)",
                [
                    'symbol' => '%'.$search.'%',
                    'description' => '%'.$search.'%'
                ]
        ...
        foreach ($r as $key => $value) {
            $value['market_exchanges'] = 'US_STOCK';
            $value['symbol_exchanges'] = $value['symbol'];            
            $value['currency_exchanges'] = 'USDT';
            $value['currency_crypto_longname'] = $value['description'];
            $value['order_ratio'] = 1;
            $value['symbol_longname'] = $value['description'];
            unset($value['symbol']);
  ...
  public function _Forexquery($search, $limit = 250){
    $resQuery = [];
    $alreadyFetched = [];
      //  echo "SELECT description, symbol FROM forexlist_krypto WHERE status = 1 AND (symbol LIKE '%'$search'%' OR description LIKE '%'$search'%'";
    $r = parent::querySqlRequest("SELECT description, symbol, currency FROM forexlist_krypto WHERE status = 1 AND (symbol LIKE :symbol OR description LIKE :description)",
            [
                'symbol' => '%'.$search.'%',
                'description' => '%'.$search.'%'
            ]
    ...
    foreach ($r as $key => $value) {
        $value['market_exchanges'] = 'FOREX';
        $value['symbol_exchanges'] = $value['symbol'];            
        $value['currency_exchanges'] = $value['currency'];
        $value['currency_crypto_longname'] = $value['description'];
        $value['order_ratio'] = 1;
        $value['symbol_longname'] = $value['description'];
        unset($value['symbol']);
