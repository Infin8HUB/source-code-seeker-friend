...
try {
    ...
    if(isset($_GET['module']) && $_GET['module'] == 'stock'){
        die(json_encode([
          'error' => 0,
          'coinlist' => $Search->_Stockquery($_GET['request']),
          'native' => ($App->_getHideMarket() ? 1 : 0)
        ]));
    } else if(isset($_GET['module']) && $_GET['module'] == 'forex'){
      die(json_encode([
        'error' => 0,
        'coinlist' => $Search->_Forexquery($_GET['request']),
        'native' => ($App->_getHideMarket() ? 1 : 0)
      ]));
    } else {
